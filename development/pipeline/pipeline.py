...
def go(config: DictConfig):
    ...
    if "data_check" in exec_steps_list:
        _ = mlflow.run(
                ...
                "main",
                parameters={
                    "data_path": os.path.join(data_model_parent_dir, config["data"]["raw_data_path"])
                },
                env_manager="local"
            )
    ...
    if "split_data" in exec_steps_list:
        _ = mlflow.run(
                ...
                "main",
                parameters={
                    'raw_data_path' : os.path.join(data_model_parent_dir, config["data"]["raw_data_path"]),
    ...
    if "model_train" in exec_steps_list:
        ...
        _ = mlflow.run(
                ...
                parameters={
                    ...
                    'model_save_path': os.path.join(data_model_parent_dir, config["model"]["model_save_path"]),
    ...
    if "performace_eval" in exec_steps_list:
        ...
        _ = mlflow.run(
                ...
                parameters={
                    ...
                    'model_path': os.path.join(data_model_parent_dir, config["model"]["model_save_path"]),
                    'output_label': config['train']['output_label'],
