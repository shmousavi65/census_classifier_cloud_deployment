...
def go(args):
    ...
    logger.info("calculating the model performance on the entire given \
                 data ...")
    precision, recall, fbeta = get_performnace_on_df(X_df, y_series,
                                                     input_pipe,
                                                     output_transformer)
    mlflow.log_metrics({
    ...
    logger.info("calculating the model performance on the slices of the \
                 given feature(s) ...")
    if slice_eval_features:
        for feature in slice_eval_features:
            ...
            for val in vals:
                ...
                precision, recall, fbeta = get_performnace_on_df(
                    sub_X_df,
                    sub_y_series,
                    input_pipe,
                    output_transformer)
                mlflow.log_metrics({
                    f"test_{str.strip(feature)}_{str.strip(val)}_precision":
                    precision,
                    f"test_{str.strip(feature)}_{str.strip(val)}_recall":
                    recall,
                    f"test_{str.strip(feature)}_{str.strip(val)}_fbeta":
                    fbeta})
